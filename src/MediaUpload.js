...
function MediaUpload() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (qrData?.external_link) {
        setQrUrl(qrData.external_link);
      }
      ...
      if (files) {
        const urls = files.map(f => ({
          ...
          url: supabase.storage
            .from('prakalp-projects')
            .getPublicUrl(`${teamData.id}/${f.name}`).data.publicUrl
        }));
        setUploadedUrls(urls);

        // Set QR if not already set
        if (!qrData?.external_link && urls.length > 0) {
          const projectUrl = `${window.location.origin}/project/${teamData.id}`;
          setQrUrl(projectUrl);
        }
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Save QR link to database
      await supabase
        .from('project_qr')
        .upsert({
          team_id: team.id,
          external_link: projectUrl,
          qr_image_url: projectUrl,
        }, { onConflict: 'team_id' });
      ...
      {!loading && team && (
        <>
          ...
          <div style={card}>
            ...
            <span style={{...tagCat, marginLeft:'8px',
          ...
          <div style={card}>
            ...
            <p style={muted}>
              Upload photos or videos of your project showcase.
              These will be visible to anyone who scans your QR code.
            </p>
            ...
            <div style={uploadZone}
              ...
              <p style={{fontWeight:'700', color:'#1a1508', marginBottom:'4px'}}>
                Click to select files
              </p>
              <p style={{fontSize:'0.8rem', color:'#8a7d65'}}>
                Photos (JPG, PNG) or Videos (MP4) — max 50MB each
              </p>
              {mediaFiles.length > 0 && (
                <p style={{marginTop:'8px', color:'#c84b1f', fontWeight:'700'}}>
            ...
            <button
              ...
              style={{...btnSolid, marginTop:'16px',
                opacity: (!mediaFiles.length || uploading) ? 0.5 : 1}}>
          ...
          {uploadedUrls.length > 0 && (
            <div style={card}>
              <h2 style={sectionTitle}>
                Uploaded Media ({uploadedUrls.length} files)
              </h2>
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px,1fr))', gap:'12px'}}>
                {uploadedUrls.map((f, idx) => (
                  <div key={idx} style={mediaThumb}>
                    ...
                    <p style={{fontSize:'0.7rem', color:'#8a7d65',
          ...
          {qrUrl && (
            <div style={{...card, textAlign:'center'}}>
              ...
              <p style={{...muted, marginBottom:'20px'}}>
                Anyone who scans this QR will see your project page
                with all uploaded photos and videos.
              </p>
              <div style={{display:'inline-block', padding:'20px',
                background:'#fff', borderRadius:'16px',
                border:'1.5px solid #ddd5c0', marginBottom:'20px'}}>
                <QRCodeCanvas
                  id="project-qr"
                  value={qrUrl}
                  size={200}
              ...
              <p style={{fontSize:'0.75rem', color:'#8a7d65', marginBottom:'20px'}}>
                {qrUrl}
              </p>
              <div style={{display:'flex', gap:'12px', justifyContent:'center', flexWrap:'wrap'}}>
                <button onClick={downloadQR} style={btnSolid}>
                  ⬇️ Download QR Code
                </button>
                <button
                  onClick={() => window.open(`/project/${team.id}`, '_blank')}
                  style={btnGhost}>
...
const wrap = {
  ...
  fontFamily:"'Cabinet Grotesk',sans-serif", color:'#1a1508',
  padding:'32px 20px'
};
const card = {
...
const sectionTitle = {
  ...
  fontWeight:'600', color:'#1a1508', marginBottom:'12px'
};
...
const uploadZone = {
  border:'2px dashed #ddd5c0', borderRadius:'12px', padding:'32px',
  textAlign:'center', cursor:'pointer', background:'#faf7f2',
  transition:'border-color 0.2s'
};
const mediaThumb = { textAlign:'center' };
const mediaEl = {
  width:'100%', height:'100px', objectFit:'cover',
  borderRadius:'8px', border:'1px solid #ddd5c0'
};
const btnSolid = {
  padding:'11px 24px', background:'#1a1508', color:'#f5f0e8',
  border:'2px solid #1a1508', borderRadius:'9px',
  fontFamily:"'Cabinet Grotesk',sans-serif",
  fontWeight:'700', fontSize:'0.875rem', cursor:'pointer'
};
const btnGhost = {
  padding:'11px 24px', background:'transparent', color:'#1a1508',
  border:'2px solid #ddd5c0', borderRadius:'9px',
  fontFamily:"'Cabinet Grotesk',sans-serif",
  fontWeight:'700', fontSize:'0.875rem', cursor:'pointer'
};
const errorBox = {
  background:'rgba(200,75,31,0.08)', border:'1px solid rgba(200,75,31,0.3)',
  borderRadius:'10px', padding:'12px 16px', color:'#c84b1f',
  marginBottom:'16px', fontSize:'0.875rem'
};
const successBox = {
  background:'rgba(45,106,79,0.08)', border:'1px solid rgba(45,106,79,0.3)',
  borderRadius:'10px', padding:'12px 16px', color:'#2d6a4f',
  marginBottom:'16px', fontSize:'0.875rem'
};
